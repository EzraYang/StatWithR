...
summarise(sfo_feb_flights, num = n())
```
...
<li> ✔️The distribution is right skewed. </li> 
...
<li> ✔️Delta and United Airlines </li>
...
sfo_feb_flights %>% 
    group_by(carrier) %>% 
    summarise(arr_delay_med = median(arr_delay), arr_delay_IQR = IQR(arr_delay)) %>% 
...
<li> ✔️Median would be more reliable as the distribution of delays is skewed. </li> 
...
nycflights %>% 
    group_by(origin) %>% 
    summarise(ot_dep_perc = sum(dep_type == 'on time') / n() ) %>% 
...
<li> ✔️There is an overall postive association between distance and average speed.</li> 
...
nycflights %>% 
    ...
    summarise( prop_dd_ao = sum( arr_type == 'on time' ) / n())
```
