...
<li> ✔️Yes, revise wording to "Is there an association between beauty and course evaluations?" </li>
...
<li> ✔️The left skewness of the data suggests that the students are less likely to rate the professors highly. </li>
</ol>
```{r score-dist}
# type your code for Question 3 here, and Knit
# View(evals)
qplot(score, data = evals, color = 'red')
```
```{r}
summary(evals$score)
```
...
I'm using gender to explain bty_avg, from the boxplot we can tell that the maximum, 3rd quantile, median, 1st quantile point of female professors are all above those of male professors.  
```{r two-vars-eda}
...
summary(m_bty)
```
The equation of the linear model is simply $$\widehat{score} = 0.06664*bty\_avg  + 3.88034$$.  
The slope means as the bty_avg increase by 1, the expected value of score goes up by 0.06664.  
...
<li> ✔️Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </li>
<li> Independent observations: Classes sampled randomly, no order effect. </li>
</ol>
```{r diag-score-bty_avg}
# type your code for the Question 5 here, and Knit
res1 <- ggplot(data = m_bty, aes(x = .fitted, y = .resid)) + 
    geom_point(alpha = 1/3) + 
    geom_hline(yintercept = 0, linetype = 'dashed') + 
    xlab('Fitted values') + 
    ylab('Residuals')

res2 <- ggplot(data = m_bty, aes(x = .fitted, y = abs(.resid))) + 
    geom_point(alpha = 1/3) + 
    xlab('Fitted values') + 
    ylab('Absolute value of residuals')

res3 <- ggplot(data = m_bty, aes(x = .resid) ) + 
    geom_histogram()

grid.arrange(res1, res2, res3, ncol = 2)
```
...
```{r diag-mlr-model}
# type your code for the Exercise here, and Knit
bty_gen_res1 <- ggplot(data = m_bty_gen, aes(sample = .resid)) + 
    stat_qq(alpha = 1/5) + 
    xlab('Theoretical quantiles') + 
    ylab('Residuals')

bty_gen_res2 <- ggplot(data=m_bty_gen, aes(x=.fitted, y=.resid)) + 
    geom_point(alpha = 1/3) + 
    geom_hline(yintercept = 0, linetype = 'dashed') + 
    xlab('Fitted value') + 
    ylab('Residuals')

bty_gen_res3 <- ggplot(data=m_bty_gen, aes(x=bty_avg, y=.resid)) + 
    geom_point(alpha = 1/3) + 
    xlab('bty_avg') + 
    ylab('Residuals')

bty_gen_res4 <- ggplot(data=m_bty_gen, aes(x=gender, y=.resid)) + 
    geom_point(alpha = 1/3) + 
    xlab('gender') + 
    ylab('Residuals')


grid.arrange(bty_gen_res1, bty_gen_res2, bty_gen_res3, bty_gen_res4, ncol=2)
```
...
With t-value increased and P-value decreased, the addition of `gender` makes `bty_avg` more significant.  
...
<li> ✔️Tenure Track, Tenured, Teaching </li>
...
<li> ✔️0.12 points higher than minority professors, all else held constant. </li> 
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_dr1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
...
The removal of `cls_profs` yeilds the highest improvement in adjusted $R^2$, from 0.1617 in full model to 0.1634262 in the model without `cls_profs`.  
...
```{r}
mm_full <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm_full)$adj.r.squared
```


```{r}
# drop rank
mm1 <- lm(score ~ ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm1)$adj.r.squared
```


```{r}
# drop ethnicity
mm2 <- lm(score ~ rank + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm2)$adj.r.squared
```


```{r}
# drop gender 
mm3 <- lm(score ~ rank + ethnicity + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm3)$adj.r.squared
```


```{r}
# drop language
mm4 <- lm(score ~ rank + ethnicity + gender + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm4)$adj.r.squared
```


```{r}
# drop age
mm5 <- lm(score ~ rank + ethnicity + gender + language + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm5)$adj.r.squared
```


```{r}
# drop cls_perc_eval
mm6 <- lm(score ~ rank + ethnicity + gender + language + age 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm6)$adj.r.squared
```


```{r}
# drop cls_students
mm7 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
              + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm7)$adj.r.squared
```


```{r}
# drop cls_level
mm8 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm8)$adj.r.squared
```


```{r}
# drop cls_credits
mm9 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + bty_avg 
             + pic_outfit + pic_color, data = evals)  
summary(mm9)$adj.r.squared
```


```{r}
# drop bty_avg
mm10 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits 
             + pic_outfit + pic_color, data = evals)  
summary(mm10)$adj.r.squared
```


```{r}
# drop pic_outfit
mm11 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_color, data = evals)  
summary(mm11)$adj.r.squared
```


```{r}
# drop pic_color
mm12 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit, data = evals)  
summary(mm12)$adj.r.squared
```
